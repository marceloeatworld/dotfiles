...
- **WM:** Hyprland (Wayland) + hyprbars plugin
...
└── modules/
    ...
    │   ├── security.nix      # Firejail (15 apps), AppArmor, GnuPG, sudo rules
    ...
    └── home/                 # Home Manager modules
        ├── home.nix          # Root config, imports 32 modules
        ├── programs/         # 20 program modules
        ...
        │   ├── nemo.nix      # Nemo file manager + custom actions
        ...
        ├── services/
        │   ├── waybar.nix    # Status bar + 15+ custom scripts
...
- **Hyprland** - Tiling Wayland compositor with hyprbars plugin
- **5 Themes** - Switch with `theme-selector` (Ristretto, Neobrutalist, Nord, Tokyo Night, Catppuccin)
- **Firejail** - 15 sandboxed apps (Brave, Wireshark, KeePassXC, LibreOffice, etc.)
...
| `SUPER + N` | Blue light filter cycle (9 levels: off → 6500K → ... → 1000K) |
...
**Auto-update functions:** `update-vscode`, `update-claude-code`, `update-llama`, `update-overlays`
...
- **Monitor rotation** - Screen rotation toggle
