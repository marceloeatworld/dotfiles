...
in
{
  ...
  virtualisation.podman = {
    enable = true;
    dockerCompat = true;      # Provides 'docker' command alias (shim calls podman)
    # No Docker daemon/socket is used on this host. Tools that need the Docker
    # API use the rootless Podman socket through DOCKER_HOST from Home Manager.
    dockerSocket.enable = false;
    defaultNetwork.settings.dns_enabled = true;  # DNS resolution between containers
    autoPrune = {
      enable = true;
      dates = "weekly";
      # --all removes every image unused by a container, not just dangling
      # layers; the default let 127GB of old build images pile up.
      flags = [ "--all" ];
    };
  };

  # Per-container CPU cap (CPUQuota on libpod-*.scope) lives in
  # modules/home/programs/development.nix; /etc/systemd/user is a symlink
  # managed by the systemd module, so it cannot be extended from here.
  ...
  virtualisation.libvirtd = {
    ...
    # Exit when idle for 2 min (no client connection, no running VM).
    # Paired with the wantedBy override below for full lazy loading.
    extraOptions = [ "--timeout" "120" ];
    qemu = {
  ...
  # Lazy-load libvirtd: not started at boot; libvirtd.socket starts it on the
  # first virsh/virt-manager connection, and --timeout above stops it again.
  # libvirt-guests with onBoot = "ignore" never contacts the daemon at boot.
