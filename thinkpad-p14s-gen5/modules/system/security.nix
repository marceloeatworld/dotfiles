  ...
  programs.firejail = {
    ...
    wrappedBinaries = {
      ...
      brave = {
        ...
        extraArgs = [
          # Restricted file access - only directories Brave needs
          "--whitelist=~/Downloads"
          "--whitelist=~/Documents"
          "--whitelist=~/Pictures"
          "--whitelist=~/Videos"
      ...
      # NOTE: wrangler is installed via bun global + sandboxed wrapper in home.nix
  ...
  security.sudo.extraRules = [
    {
      ...
      commands = [
        ...
        {
          command = "/nix/store/*/bin/ln -sf /home/marcelo/models/* /var/lib/llama-cpp/*";
          options = [ "NOPASSWD" ];
        }
        # Vision model (OCR) drop-in management for llama-cpp
        {
          command = "/nix/store/*/bin/mkdir -p /run/systemd/system/llama-cpp.service.d";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/nix/store/*/bin/tee /run/systemd/system/llama-cpp.service.d/override.conf";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/nix/store/*/bin/rm -rf /run/systemd/system/llama-cpp.service.d";
          options = [ "NOPASSWD" ];
        }
        {
