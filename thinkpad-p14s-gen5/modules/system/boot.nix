...

{
  ...
  boot.kernelParams = [
    ...
    "amdgpu.gfx_off=0"                # Disable GFXOFF - causes resume failures on RDNA 3 iGPU (black screen after suspend)
    ...
    # Source: ArchWiki ThinkPad P14s Gen 5 AMD (still recommended as of kernel 6.18)
    "amdgpu.dcdebugmask=0x410"

    # Additional stability parameters for AMD display
    "amdgpu.sg_display=0"              # Fixes screen flickering and TLB fence timeouts (RDNA 3 iGPU)
    "amdgpu.runpm=0"                   # Disable GPU runtime PM - prevents suspend/resume crashes on iGPU

    # NOTE: cwsr_enable=0 may be needed if GPU hard hangs occur on kernel 6.18+
    # Known RDNA 3 regression: CWSR saturates MES ring buffer causing system freeze
    # Partial fixes in 6.18.4 and 6.18.14+, fully fixed in 6.18.17+
    # Uncomment if experiencing hard hangs under GPU compute workloads:
