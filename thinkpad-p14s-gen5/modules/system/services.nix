...
let
  ...
  llm-switch = pkgs.writeShellScriptBin "llm-switch" ''
    ...
    current=""
    if [ -L "$SYMLINK" ]; then
      current=$(${pkgs.coreutils}/bin/basename "$(${pkgs.coreutils}/bin/readlink "$SYMLINK")")
    fi
    ...
    case "''${1:-}" in
      ...
      status|"")
        ...
        echo ""
        echo "Usage: llm-switch [4b|9b|status|stop]"
        exit 0
        ;;
      stop)
        echo "Stopping llama-cpp..."
        sudo /run/current-system/sw/bin/systemctl stop llama-cpp
        echo "LLM stopped."
        exit 0
        ;;
      *)
    ...
    sudo ${pkgs.coreutils}/bin/ln -sf "$MODELS_DIR/$target" "$SYMLINK"
    # Also symlink the chat template if present
    if [ -f "$MODELS_DIR/qwen3.5-no-think.jinja" ]; then
      sudo ${pkgs.coreutils}/bin/ln -sf "$MODELS_DIR/qwen3.5-no-think.jinja" "$SERVICE_DIR/qwen3.5-no-think.jinja"
    fi
    sudo /run/current-system/sw/bin/systemctl restart llama-cpp
