...
in
{
  ...
  config = {
    ...
    # AMD GPU/CPU + WiFi firmware all ship in linux-firmware (redistributable);
    # no non-redistributable blob is needed on this hardware, so the narrower
    # option keeps the closure smaller than enableAllFirmware.
    ...
    # NOTE: initcall_blacklist=acpi_cpufreq_init removed - amd-pstate active mode comes from the nixos-hardware P14s profile (see boot.nix)
    ...
    # NOTE: linux-firmware is pulled in by hardware.enableRedistributableFirmware above.
    ...
    environment.systemPackages = with pkgs; [
      ...
      # NOTE: LACT is in nixpkgs (pkgs.lact, services.lact); intentionally not installed:
      # lact

      # Power monitoring
      powertop

      # Storage health (smartd service configured in services.nix)
      smartmontools # smartctl
      nvme-cli # NVMe-specific health/error-log queries
