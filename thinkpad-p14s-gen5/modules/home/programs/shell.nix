...
in
{
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      update-apps = "update-overlays && nh os switch || echo 'Nothing to update.'";  # Updates VS Code & Claude Code (will close running instances)
      ...
      # AI/LLM
      # llm is a script in development.nix (llama-cli direct), aichat is separate
      # Shortcuts
      v = "nvim";
      vim = "nvim";
      ".." = "cd ..";
    ...
    initContent = ''
      ...
      # OpenCode auto-update function (fetches latest from GitHub releases)
      function update-opencode() {
        local OVERLAY="$HOME/dotfiles/thinkpad-p14s-gen5/overlays/opencode-latest.nix"

        echo ""
        echo "══════════════════════════════════════════"
        echo "  OpenCode Update Check"
        echo "══════════════════════════════════════════"

        local LATEST=$(curl -s "https://api.github.com/repos/anomalyco/opencode/releases/latest" 2>/dev/null | jq -r '.tag_name' | sed 's/^v//')
        local CURRENT=$(grep 'version = ' "$OVERLAY" | sed 's/.*"\(.*\)".*/\1/')
        ...
        echo "✓ Updated to OpenCode $LATEST"
        return 2
      }

      # Update quick overlays (VS Code + Claude Code + OpenCode)
