...
let
  ...
  protectSensitiveFilesHook = pkgs.writeText "protect_sensitive_files.py" ''
    ...
    SENSITIVE_DIRS = frozenset({
        ...
        '.kube', '.docker', '.config/containers', '.helm',
        # Password managers
  ...
  # Gemini API Skills (from google-gemini/gemini-skills via skills.sh)
  geminiApiDevSkill = builtins.readFile ../../../skills/gemini-api-dev.md;
  geminiLiveApiDevSkill = builtins.readFile ../../../skills/gemini-live-api-dev.md;
  geminiInteractionsApiSkill = builtins.readFile ../../../skills/gemini-interactions-api.md;
...
in
{
  ...
  # Deploy Gemini API skills (universal + Claude Code symlink)
  home.file.".agents/skills/gemini-api-dev/SKILL.md" = {
    text = geminiApiDevSkill;
  };
  home.file.".claude/skills/gemini-api-dev/SKILL.md" = {
    text = geminiApiDevSkill;
  };

  home.file.".agents/skills/gemini-live-api-dev/SKILL.md" = {
    text = geminiLiveApiDevSkill;
  };
  home.file.".claude/skills/gemini-live-api-dev/SKILL.md" = {
    text = geminiLiveApiDevSkill;
  };

  home.file.".agents/skills/gemini-interactions-api/SKILL.md" = {
    text = geminiInteractionsApiSkill;
  };
  home.file.".claude/skills/gemini-interactions-api/SKILL.md" = {
    text = geminiInteractionsApiSkill;
  };

  # Install/update Clerk AI skills (from https://github.com/clerk/skills)
  home.activation.installClerkSkills = config.lib.dag.entryAfter ["writeBoundary"] ''
    CLERK_DIR="$HOME/.claude/skills/clerk"
    if [ -d "$CLERK_DIR/.git" ]; then
      $DRY_RUN_CMD ${pkgs.git}/bin/git -C "$CLERK_DIR" pull --ff-only 2>/dev/null || true
    else
      $DRY_RUN_CMD ${pkgs.bash}/bin/bash -c "rm -rf '$CLERK_DIR' && ${pkgs.git}/bin/git clone https://github.com/clerk/skills '$CLERK_DIR'" 2>/dev/null || true
    fi
  '';

  # Install/update official Cloudflare skills (from https://github.com/cloudflare/skills)
  home.activation.installCloudflareSkills = config.lib.dag.entryAfter ["writeBoundary"] ''
    CF_CACHE="$HOME/.cache/claude/cloudflare-skills"
    CF_SKILLS="$HOME/.claude/skills"
    CF_COMMANDS="$HOME/.claude/commands"

    # Clone or update official repo
    if [ -d "$CF_CACHE/.git" ]; then
      $DRY_RUN_CMD ${pkgs.git}/bin/git -C "$CF_CACHE" pull --quiet 2>/dev/null || true
    else
      $DRY_RUN_CMD rm -rf "$CF_CACHE"
      $DRY_RUN_CMD mkdir -p "$(dirname "$CF_CACHE")"
      $DRY_RUN_CMD ${pkgs.git}/bin/git clone --quiet https://github.com/cloudflare/skills "$CF_CACHE"
    fi

    # Copy all skills
    if [ -d "$CF_CACHE/skills" ]; then
      for skill_dir in "$CF_CACHE/skills/"*/; do
        [ -d "''${skill_dir}" ] || continue
        skill_name=$(${pkgs.coreutils}/bin/basename "''${skill_dir}")
        $DRY_RUN_CMD rm -rf "$CF_SKILLS/''${skill_name}"
        $DRY_RUN_CMD cp -r "''${skill_dir}" "$CF_SKILLS/''${skill_name}"
      done
    fi

    # Copy commands
    if [ -d "$CF_CACHE/commands" ]; then
      $DRY_RUN_CMD mkdir -p "$CF_COMMANDS"
      $DRY_RUN_CMD cp -r "$CF_CACHE/commands/"* "$CF_COMMANDS/" 2>/dev/null || true
    fi
  '';

  # Install/update official Supabase skills (from https://github.com/supabase/agent-skills)
  home.activation.installSupabaseSkills = config.lib.dag.entryAfter ["writeBoundary"] ''
    SB_CACHE="$HOME/.cache/claude/supabase-skills"
    SB_SKILLS="$HOME/.claude/skills"

    if [ -d "$SB_CACHE/.git" ]; then
      $DRY_RUN_CMD ${pkgs.git}/bin/git -C "$SB_CACHE" pull --quiet 2>/dev/null || true
    else
      $DRY_RUN_CMD rm -rf "$SB_CACHE"
      $DRY_RUN_CMD mkdir -p "$(dirname "$SB_CACHE")"
      $DRY_RUN_CMD ${pkgs.git}/bin/git clone --quiet https://github.com/supabase/agent-skills "$SB_CACHE"
    fi

    if [ -d "$SB_CACHE/skills" ]; then
      $DRY_RUN_CMD mkdir -p "$SB_SKILLS"
      for skill_dir in "$SB_CACHE/skills/"*/; do
        [ -d "''${skill_dir}" ] || continue
        skill_name=$(${pkgs.coreutils}/bin/basename "''${skill_dir}")
        $DRY_RUN_CMD rm -rf "$SB_SKILLS/''${skill_name}"
        $DRY_RUN_CMD cp -r "''${skill_dir}" "$SB_SKILLS/''${skill_name}"
      done
    fi
  '';
