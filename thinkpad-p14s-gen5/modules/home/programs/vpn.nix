...
let
  vpn-script = pkgs.writeShellScriptBin "vpn" ''
    ...
    # Derive the code with get_country_code so lookup always agrees with how
    # import_configs named the connection (usa-US-TX-751.conf imports as
    # proton-tx, so only "vpn tx" matches it - a raw -US- grep would not).
    find_config() {
      local want
      want=$(echo "$1" | tr '[:upper:]' '[:lower:]')
      for conf in "$VPN_DIR"/*.conf; do
        [ -f "$conf" ] || continue
        if [ "$(get_country_code "$conf")" = "$want" ]; then
          echo "$conf"
          return 0
        fi
      done
