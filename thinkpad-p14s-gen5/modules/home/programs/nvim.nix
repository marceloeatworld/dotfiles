...

{
  programs.neovim = {
    ...
    plugins = with pkgs.vimPlugins; [
      ...
      aerial-nvim                  # Code outline panel (right sidebar, VSCode-like)
    ...
    extraPackages = with pkgs; [
      # NOTE: nil, pyright, clang-tools, typescript-language-server, gcc
      # are provided globally via development.nix home.packages

      # ══════════════════════════════════════════════════════════════════
      # LSP SERVERS (nvim-specific)
      # ══════════════════════════════════════════════════════════════════
      lua-language-server
      ...
      # TOOLS
      # ══════════════════════════════════════════════════════════════════
      nodejs-slim_22               # Required by Copilot; keep npm/npx out of PATH
    ...
    initLua = ''
      ...
      opt.pumheight = 12             -- Keep completion menu compact
      ...
      opt.timeoutlen = 350           -- Time to wait for mapping (for which-key)
      ...
      -- Big files: avoid expensive diagnostics, folding and undo churn.
      local bigfile_group = vim.api.nvim_create_augroup("BigFile", { clear = true })
      vim.api.nvim_create_autocmd("BufReadPre", {
        group = bigfile_group,
        callback = function(args)
          local uv = vim.uv or vim.loop
          local ok, stat = pcall(uv.fs_stat, args.file)
      ...
      vim.api.nvim_create_autocmd("LspAttach", {
        group = bigfile_group,
        callback = function(args)
          if vim.b[args.buf].bigfile and args.data and args.data.client_id then
            pcall(vim.lsp.buf_detach_client, args.buf, args.data.client_id)
          end
        end,
      })

      local general_group = vim.api.nvim_create_augroup("GeneralUX", { clear = true })
      ...
      -- COLORSCHEME
      -- ════════════════════════════════════════════════════════════════════
      local nvim_scheme = "${config.theme.appearance.nvimColorscheme}"
      local nvim_flavor = "${config.theme.appearance.nvimFlavor}"

      if nvim_scheme == "catppuccin" then
        require("catppuccin").setup({
          flavour = nvim_flavor ~= "" and nvim_flavor or "mocha",
      ...
      elseif nvim_scheme == "tokyonight" then
        require("tokyonight").setup({
          style = nvim_flavor ~= "" and nvim_flavor or "night",
          transparent = false,
      ...
      elseif nvim_scheme == "nord" then
        ...
        require("monokai-pro").setup({
          filter = nvim_flavor ~= "" and nvim_flavor or "ristretto",
          transparent_background = false,
          devicons = true,
          styles = {
            comment = { italic = true },
      ...
      -- TypeScript is opt-in here. VS Code is the default TypeScript owner;
      -- otherwise each Neovim terminal can spawn its own tsserver per repo.
      if vim.env.NVIM_ENABLE_TS_LSP == "1" then
        servers.ts_ls = {}
      end
      ...
      require("scrollbar").setup({
        handle = { color = "${config.theme.colors.surface}" },
        marks = {
          Search = { color = "${config.theme.colors.orange}" },
          Error = { color = "${config.theme.colors.red}" },
          Warn = { color = "${config.theme.colors.yellow}" },
          Info = { color = "${config.theme.colors.cyan}" },
          Hint = { color = "${config.theme.colors.green}" },
          Misc = { color = "${config.theme.colors.magenta}" },
        },
      })

      require('notify').setup({
        background_colour = "${config.theme.colors.background}",
        timeout = 3000,
      })
      ...
      -- Aerial (code outline panel on the right, VSCode-like)
      ...
      -- VSCode-style layout toggles
      keymap("n", "<C-b>", "<cmd>Neotree toggle<CR>", { desc = "Toggle sidebar (explorer)" })
      keymap("n", "<C-p>", "<cmd>Telescope find_files<CR>", { desc = "Quick open file" })
      ...
      wk.add({
        ...
        { "<leader>uo", "<cmd>AerialToggle!<CR>", desc = "Toggle outline (right panel)" },
...
║ Terminal: llm "prompt" → Chat   llm-switch qwythos/gemma/qwopus     ║
╠══════════════════════════════════════════════════════════════════════╣
║ GITHUB (Octo.nvim)                                                   ║
...
╚══════════════════════════════════════════════════════════════════════╝
      ...
      -- Auto commands
      -- ─────────────────────────────────────────────────────────────
      -- Yank highlight, cursor restore and q-close live in the GeneralUX augroup above

      -- Auto-open neo-tree on the left when opening a real file
      vim.api.nvim_create_autocmd("VimEnter", {
        callback = function()
          if vim.fn.argc() == 1 then
            local arg = vim.fn.argv(0)
            if type(arg) == "string" and vim.fn.filereadable(arg) == 1 then
              vim.schedule(function()
