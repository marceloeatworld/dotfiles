...
in
{
  ...
  # === NEMO GTK CSS ===
  # Runtime theme import. The actual colors live under
  # ~/.config/theme/current so Nemo follows day/night without a rebuild.
  home.file.".config/gtk-3.0/nemo.css".text = ''
    @import url("${config.home.homeDirectory}/.config/theme/current/nemo.css");
  '';
}
