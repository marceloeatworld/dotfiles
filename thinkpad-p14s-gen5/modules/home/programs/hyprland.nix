  ...
  perf-mode-daemon = pkgs.writeShellScriptBin "perf-mode-daemon" ''
    ...
    ${pkgs.upower}/bin/upower --monitor-detail | while read -r line; do
      if echo "$line" | grep -q "state:"; then
        state=$(echo "$line" | sed 's/.*state:\s*//' | xargs)
        case "$state" in
...
<tt>Super+Shift+Esc</tt> Power off
<tt>Super+Ctrl+Esc</tt>  Reboot
  ...
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      exec-once = [
        ...
        "uwsm app -- hyprlauncher -d"  # Start hyprlauncher daemon
        "uwsm app -- hypridle"
      ...
      cursor = {
        no_hardware_cursors = true;  # Fix SEGV crash in renderHWCursorBuffer on AMD RDNA 3 with Hyprland 0.54
        ...
        enable_hyprcursor = false;  # Disabled: Bibata is XCursor-only, hyprcursor crashes on fallback in 0.54
      };
