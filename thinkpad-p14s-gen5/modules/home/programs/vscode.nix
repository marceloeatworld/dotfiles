...
in
{
  programs.vscode = {
    ...
    profiles.default = {
      ...
      extensions = with pkgs.vscode-extensions; [
        # C# / .NET official Microsoft tooling. Keep the heavyweight C# Dev Kit
        # out of the baseline because it triggers Microsoft Account sign-in.
        # ms-dotnettools.vscode-dotnet-runtime must NOT come from nixpkgs: it
        # chmods and runs install scripts inside its own extension dir, which
        # fails on the read-only store (EROFS). The marketplace copy in the
        # mutable extensions dir handles .NET runtime downloads fine.
        ms-dotnettools.csharp

        # C / C++ official and community-standard tooling.
        ms-vscode.cpptools
  ...
  home.file = {
    ...
    # Absolute path key intentionally matches the home.file attribute name that
    # programs.vscode generates, so .force merges into it. A relative key here
    # creates a second managed entry for the same target -> activation conflict.
  ...
  # NOTE: Electron then migrates itself into a self-created transient
  # "app-code-<pid>.scope" under app.slice, escaping the slice. The drop-in
  # below is what actually enforces the cap; the wrapper still handles the
  # Wayland env and covers the short-lived CLI process.
  home.packages = [
    (lib.hiPrio (pkgs.writeShellScriptBin "code" ''
      exec ${pkgs.systemd}/bin/systemd-run --user --quiet --scope \
        --slice=app-code.slice \
        ${pkgs.coreutils}/bin/env -u NIXOS_OZONE_WL \
          ELECTRON_OZONE_PLATFORM_HINT=wayland \
          ${pkgs.vscode}/bin/code "$@"
    ''))
  ];

  # systemd searches dash-truncated drop-in dirs (app-code-.scope.d matches
  # app-code-<pid>.scope), and transient units honor drop-ins. This caps the
  # scopes VS Code creates for itself, whatever the launch path.
  # Keep values in sync with app-code.slice in modules/system/performance.nix.
  xdg.configFile."systemd/user/app-code-.scope.d/50-memory-cap.conf".text = ''
    [Scope]
    MemoryHigh=10G
    MemoryMax=14G
    MemorySwapMax=8G
