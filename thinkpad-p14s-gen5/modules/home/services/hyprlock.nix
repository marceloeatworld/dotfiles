...
in
{
  ...
  xdg.configFile."hypr/hypridle.conf".text = ''
    ...
    general {
      ...
      after_sleep_cmd = sleep 4 && hyprctl eval "hl.dispatch(hl.dsp.dpms({ action = 'on' }))"
    }
    ...
    # On battery: suspend after 45 minutes idle. Fullscreen/video windows can
    # still inhibit idle through the Hyprland window rules.
    listener {
      timeout = 2700
  ...
  xdg.configFile."hypr/hyprlock.conf".text = ''
    ...
    # hyprlock 0.9.5: disable_loading_bar/grace/no_fade_in/no_fade_out removed;
    # grace = 0 is the default (--grace CLI arg otherwise), fades moved to animations
    general {
      hide_cursor = true
    }
