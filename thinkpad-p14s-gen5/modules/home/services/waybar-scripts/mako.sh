...
notif_data=$(makoctl history -j 2>/dev/null)
notif_count=$(echo "$notif_data" | jq 'length' 2>/dev/null || echo "0")
...
if [ "$notif_count" -gt 0 ]; then
  ...
  mapfile -t apps < <(
    echo "$notif_data" |
      jq -r '
        .[0:10]
        | group_by(.app_name // "Unknown")[]
        | [length, (.[0].app_name // "Unknown")]
        | @tsv
      ' 2>/dev/null |
      sort -rn
  )
  ...
  for entry in "${apps[@]}"; do
    ...
    [ -z "$app" ] && continue
    last_msg=$(echo "$notif_data" | jq -r --arg app "$app" '.[] | select((.app_name // "Unknown") == $app) | .summary' 2>/dev/null | head -1)
