...
- Python dependencies are provided by Nix (see waybar.nix), no runtime installs

USAGE MODES:
1. Normal mode (default):
   python wallets.py
   → Uses cached balances, updates price only (fast, runs every 10 min)
...
CACHE_DURATION = timedelta(hours=24)  # Unused: address cache never expires (see load_cache); price refreshes each run (waybar interval: 10 min)
...
def ensure_dependencies():
    """Check Nix-provided Python dependencies."""
    try:
        import embit
        import requests
        return True
    except ImportError as e:
        print(f"Missing Python dependency from Nix environment: {e}", file=sys.stderr)
        return False
...
def main():
    ...
    if not ensure_dependencies():
        print(json.dumps({
            "text": "",
            "tooltip": "Missing Nix Python dependency\nRun nh os switch after updating",
            "class": "error"
        }))
        return
    ...
    # Build tooltip with Pango markup (consistent with other waybar scripts)
    lines = [h("  WALLETS"), ""]

    # Total balance
    lines.append(f"{d('BTC')}  {v(f'{total_btc:.8f}')}")
    lines.append(f"{d('USD')}  {v(f'${total_usd:,.2f}')}")
    lines.append(f"{d('EUR')}  {v(f'€{total_eur:,.2f}')}")

    # Individual wallets
    if len(wallets) > 1:
        lines.append("")
        lines.append(h("  BREAKDOWN"))
        lines.append("")
        ...
        for wallet_id, wallet in sorted(wallets.items()):
            ...
            bal = wallet['balance']
            lines.append(v(wallet['name']))
            lines.append(f"  {d('BTC')} {v(f'{bal:.8f}')}  {d('$')}{v(f'{wallet_usd:,.0f}')}  {d('€')}{v(f'{wallet_eur:,.0f}')}")

    # Price reference
    lines.append("")
    lines.append(h("  PRICE"))
    lines.append("")
    lines.append(f"{d('USD')}  {v(f'${usd_price:,.0f}')}")
    lines.append(f"{d('EUR')}  {v(f'€{eur_price:,.0f}')}")

    # Privacy note
    lines.append("")
    lines.append(g("  Keys never leave your machine"))
