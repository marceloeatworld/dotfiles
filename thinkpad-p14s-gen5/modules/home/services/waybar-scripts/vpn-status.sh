...
# Shows country flag emoji + country code in bar; latency shown in tooltip

PUBLIC_IP_CACHE="$HOME/.cache/vpn-public-ip"
VPN_STATE_CACHE="$HOME/.cache/vpn-state"
LATENCY_CACHE="$HOME/.cache/vpn-latency"
PUBLIC_IP_CACHE_DURATION=600  # seconds; must exceed the 60s module interval (state changes invalidate the cache anyway)
LATENCY_CACHE_DURATION=300   # seconds
...
# Check if VPN state changed - if so, invalidate IP and latency caches
previous_state=$(cat "$VPN_STATE_CACHE" 2>/dev/null || echo "unknown")
if [ "$current_vpn_state" != "$previous_state" ]; then
  rm -f "$PUBLIC_IP_CACHE" "$LATENCY_CACHE" 2>/dev/null
...
if [ -n "$vpn_interface" ]; then
  ...
  # Extract country code: proton-<cc> naming convention first (lowercase, see vpn.nix)
  country=$(printf '%s' "$vpn_name" | sed -n 's/^proton-\([a-zA-Z]\{2\}\)$/\1/p' | tr '[:lower:]' '[:upper:]')
  [ -z "$country" ] && country=$(echo "$vpn_name $device" | grep -oE '[^a-zA-Z]([A-Z]{2})[^a-zA-Z]' | grep -oE '[A-Z]{2}' | head -1 || true)
  ...
  # Latency: cached, refreshed in background (no blocking ping on every run)
  latency_valid=false
  if [ -f "$LATENCY_CACHE" ]; then
    latency_age=$(($(date +%s) - $(stat -c %Y "$LATENCY_CACHE" 2>/dev/null || echo 0)))
    [ "$latency_age" -lt "$LATENCY_CACHE_DURATION" ] && latency_valid=true
  fi
  if ! $latency_valid && [ -w "$(dirname "$LATENCY_CACHE")" ]; then
    (
      tmp="${LATENCY_CACHE}.$$"
      ping -I "$device" -c1 -W2 1.1.1.1 2>/dev/null | grep -oP 'time=\K[\d.]+' > "$tmp"
      [ -s "$tmp" ] && mv "$tmp" "$LATENCY_CACHE" || rm -f "$tmp"
    ) &
  fi
  if [ -s "$LATENCY_CACHE" ]; then
    latency="$(cat "$LATENCY_CACHE")ms"
  else
    latency="N/A"
  fi
