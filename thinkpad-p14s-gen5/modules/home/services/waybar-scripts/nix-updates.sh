...
# Overlay versions
cc=$(pango_escape "$(grep 'version = ' overlays/claude-code-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
vs=$(pango_escape "$(grep 'version = ' overlays/vscode-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
oc=$(pango_escape "$(grep 'version = ' overlays/opencode-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
fc=$(pango_escape "$(grep 'version = ' overlays/forgecode-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
cx=$(pango_escape "$(grep 'version = ' overlays/codex-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
rp=$(pango_escape "$(grep 'version = ' overlays/runpodctl-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
pn=$(pango_escape "$(grep 'version = ' overlays/pnpm-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
ll=$(pango_escape "$(grep 'version = ' overlays/llama-cpp-latest.nix 2>/dev/null | head -1 | sed 's/.*"\(.*\)".*/\1/')")
# Waybar tracks master; show the running binary's base version instead of the git rev
wb=$(pango_escape "$(waybar --version 2>/dev/null | head -1 | sed 's/^Waybar v\{0,1\}//; s/[ -].*//')")
[ -n "$wb" ] || wb="0.15.0"
...
ov="$ov\n<span color='$C_DIM'>ForgeCode</span>    <span color='$C_FG'>$fc</span>"
ov="$ov\n<span color='$C_DIM'>Codex</span>        <span color='$C_FG'>$cx</span>"
ov="$ov\n<span color='$C_DIM'>RunPod CLI</span>   <span color='$C_FG'>$rp</span>"
ov="$ov\n<span color='$C_DIM'>pnpm</span>         <span color='$C_FG'>$pn</span>"
ov="$ov\n<span color='$C_DIM'>llama.cpp</span>    <span color='$C_FG'>b$ll</span>"
ov="$ov\n<span color='$C_DIM'>Waybar</span>       <span color='$C_FG'>$wb</span>"
