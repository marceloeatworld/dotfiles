...
let
  ...
  wrangler-wrapper = pkgs.writeShellScriptBin "wrangler" ''
    export PATH="${pkgs.nodejs-slim_22}/bin:$PATH"
  ...
  theme-selector = pkgs.writeShellScriptBin "theme-selector" ''
        ...
        DOTFILES_DIR="$(find_dotfiles_dir)" || exit 1
...
in
{
  ...
  # home-manager master and nixpkgs nixos-unstable are both on 26.11 now, so
  # this is currently a no-op; kept because home-manager master bumps its
  # version first at every release cycle and the skew warning would return.
  ...
  imports = [
    ...
    ./programs/hyprland # Hyprland config (split into sub-modules)
    ./programs/hyprlauncher.nix # Application launcher
    ./programs/wofi.nix # dmenu-like picker (clipboard history, prompts)
    ./programs/uwsm.nix # Universal Wayland Session Manager

    # ── Terminal & Shell ──
    inputs.nix-index-database.homeModules.nix-index # prebuilt nix-index DB (comma / `,`)
    ./programs/terminal.nix # Ghostty + Alacritty terminals
    ./programs/xfeed # Removable transparent X timeline panel (Super+R)
    ./programs/shell.nix # Zsh + Starship prompt
    ./programs/overlay-updates.nix # update-* overlay helper functions (zsh)
    ...
    ./programs/jailed-agents.nix # Opt-in sandboxed wrappers: jailed-*
    ...
    ./services/mic-jack-autoswitch.nix # Auto-switch mic on jack plug/unplug

    # ── Configuration ──
    ./config/theme.nix # Centralized theme system (colors + fonts)
    ./config/runtime-theme.nix # Runtime day/night theme switcher (no rebuild)
    ./config/gtk.nix # GTK theme (Adwaita-dark)
    ./config/qt.nix # Qt theme
    ./config/fontconfig.nix # Font configuration
    ./config/bat.nix # Bat (cat replacement) + themed tmTheme
  ...
  home.sessionVariables = {
    ...
    # Do not set CONTAINER_HOST globally. Podman treats it as remote-client
    # mode, which prevents the user podman.service from starting its API socket.
  ...
  home.packages = with pkgs; [
    ...
    fd
    eza
    # NOTE: bat is managed by programs.bat.enable in config/bat.nix
    ...
    # GPU cloud
    runpodctl # RunPod CLI - manage GPU pods/serverless (v2, prebuilt binary via overlay)
    ...
    # Messaging
    ferdium # Multi-account workspace for WhatsApp + WhatsApp Business
    vesktop # Discord client with Vencord (replaces discord-web webapp)
    halloy # IRC client (Rust GUI, native Wayland)
