...
let
  ...
  secrets-setup = pkgs.writeShellScriptBin "secrets-setup" ''
    ...
    echo "  GLM_API_KEY, ZHIPU_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY,"
...
in
{
  ...
  sops = {
    ...
    secrets = {
      ...
      glm_api_key = {};         # Z.AI GLM (OpenCode/ForgeCode)
  ...
  programs.zsh.initContent = ''
    ...
    load-ai-secrets() {
      ...
      __load_sops_secret ZHIPU_API_KEY "${config.sops.secrets.glm_api_key.path}"
      __load_sops_secret ZAI_API_KEY "${config.sops.secrets.glm_api_key.path}"
    ...
    unload-ai-secrets() {
      unset GEMINI_API_KEY GLM_API_KEY ZHIPU_API_KEY ZAI_API_KEY MINIMAX_API_KEY OPENAI_API_KEY OPENROUTER_API_KEY FAL_KEY HF_TOKEN GITHUB_PERSONAL_ACCESS_TOKEN TFE_TOKEN
    }
