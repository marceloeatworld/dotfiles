...
# Line 23: Change device = "/dev/nvme0n1";
...
# NixOS version
nixos-version
# → 26.05.20260523.2991645 or newer unstable snapshot
...
# Update flake inputs + quick overlays + rebuild
update
...
### 4. Launch desktop and web apps:

```bash
# Hyprlauncher -> type "Ferdium", "Spotify", "Vesktop", "Claude"
SUPER + D
...
In Ferdium, add WhatsApp twice if needed: one service named "WhatsApp" and one named "WhatsApp Business".

### 5. Btrfs Snapshots:

```bash
# List snapshots (automatic hourly, 24 hourly + 3 daily retained)
ls /.snapshots/home
ls /.snapshots/root
